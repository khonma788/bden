## 概要
bden.min.js は、自サイトに電話機能を簡単に導入できるJavaScriptライブラリです、基本！電話番号以外は同じ値になります、CSSと画像は固定URLを参照しますので、ご利用者様の運用サイトに一切干渉いたしません

## 使い方
以下ソースは、httpsを対象とした記載の一例です、モジュールバージョンはより新しいものをお使いください！記載の折はコピペではなく必ず…ダウンロードリンクをお使いください！尚…「設置場所・ファイル名」は問いませんが…ひとつの電話番号にひとつの配置を心がけてください
```html
<HTML>
<HEAD>
