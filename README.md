## 概要
bden.min.js は、自サイトに電話機能を簡単に導入できるJavaScriptライブラリです、電話番号が必要になります、作業前に運用グループのHPから取得願います、CSSと画像は固定URLを参照しますので、ご利用者様の運用サイトに一切干渉いたしません

## 使い方
以下ソースは、httpsを対象とした記載の一例です、モジュールバージョンは最新をお使いください！以下のコピペではなく必ず…ダウンロードリンクをお使いください！尚…ソース記載に関し「設置場所・ファイル名」は問いませんが…ひとつの電話番号にひとつの配置を心がけてください
```html
<HTML>
<HEAD>
...
### 部品
押下者が電話機を押下すると、３秒間隔でリフレッシュ描画がなされて、通話中も続きます、受話器を置いてようやく終了します
* div
上記をご覧ください
* scriptタグ
上記をご覧ください
...
* 運用サイト以外で<br>
無料ホームページへサンプルをそのまま貼り付けるのが手っ取り早いです
